...
Accurate estimation of winter surface mass balance is critical for correctly simulating the summer and overall mass balance of a glacier \citep{Reveillet2016}. Effectively representing spatial distribution of snow is also important for simulating snow and ice melt as well as energy and mass exchange between the land and atmosphere to better monitor surface runoff and its downstream effects \citep{Clark2011}. Snow distribution is sensitive to a number of complex process that partially depend on glacier location, topography, and orientation \citep{Bloschl1991, Mott2008, Clark2011, Sold2013}. Current models are not able to fully represent these processes so the distribution of snow in remote, mountainous locations is not well known. There is, therefore, a significant source of uncertainty that undermines the ability of models to represent current glacier conditions and make predictions of glacier response to a warming climate. 
 ...
 geodetic mass balance has been compared with the
mass balance estimated from direct glaciological field
measurements in a number of studies, and the differences
are often found to be large (e.g. Krimmel, 1999; Østrem and
Haakensen, 1999; Cogley, 2009; Haug and others, 2009).
The discrepancy is currently unresolved and may stem from
errors in either method. Due to the large reported dis-
crepancies, it is important to assess the uncertainties of both
measurement metho

importance of winter balance + definition
...
There is currently a disparity in snow survey sophistication between snow science and glaciology, especially for interpolating measurements and quantifying survey error. Snow measurement techniques, including direct measurement, lidar/photogrammerty and ground penetrating radar, are widely applied in both glacierized and non-glacierzed basins. However, the spatial distribution of measurements is often more limited on glaciers due to hazardous terrain. Interpolation of measurements is generally restricted to linear regressions within the glaciology literature and often only a few topographic parameters, with elevation being the most common, included in the regression. Other techniques include attributing measured values to elevation bands and kriging. Snow science literature has also applied non-linear interpolation techniques, with regression tree interpolation being the most popular. Physical snow models, such as Alpine3D and SnowDrift3D have been developed almost exclusively for non-glacierized terrain and are rarely applied for winter balance studies \citep[c.f.][]{Mott2008}.

Current approaches
-snow science
interp     -> regression, non linear methods (regression tree), physical models (SNOWBLOW etc)
errors -> measurement and snow survey \citep{Turcan1975,Woo1978, Deems2006,Trujillo2015}

-glaciology
interp 	   -> hand contour, regression (elevation, sometimes more), physical model (one)
uncertainty-> none for direct observation of winter balance
