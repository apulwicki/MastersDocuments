...
G4 & 0.0204 & 0.0086 & 0.0213 &  & 0.0086 & 0.1405 \\
G2 & 0.0263 & 0.0180 & 0.0309 &  & 0.0256 & 0.1378 \\
G13 & 0.0112 & 0.0112 & 0.0280 &  & 0.0116 & 0.0965
\end{tabular}
\end{table}
...
A major assumption in our study is that RMSE is an appropriate error metric with which we can choose best models and assess their quality. MORE HERE!!!!
...
Extensions to this work could include an investigation of experimental design, examining implications of a non-linear SWE elevation trend, and examining the effects of DEM grid size on WSMB. 

Our sampling design was chosen to extensively sample the ablation area and is likely too finely resolved for many future mass balance surveys to replicate. Therefore, it is valuable to investigate how best to reduce our sampling design and measurement spacing while maintaining a reasonable estimate of distributed WSMB. Determining a sampling design that minimizes error and reduces the number of measurements would contribute to optimizing snow surveys in mountainous regions. 
...
DEM grid cell size affects the resolution of topographic features, which can have implications for calculating a LR. 

- Accumulation variability is often due to small scale feature in complex terrain 
- Parameters designed for slowly changing topography 
- Discontinuities between grid cells

\citep{Zhang1994} 
DEM grid size significantly affects computed topographic parameters and hydrographs
...
\citep{Garbrecht1994}
- focus on extracting drainage channels but still
- DEM resolution is investigated for a 84km 2
watershed in southwestern Oklahoma. Grid resolution
is decreased by grid cell aggregation of a
30 x 30 m baseline DEM,
- The sensitivity analysis
suggests that a DEM should have a grid area less than
5\% of the network reference area to reproduce
important drainage features with an accuracy of
about 10\%. 

\citep{Wechsler2006}
- This paper describes a Monte Carlo methodology for evaluation of the effects of uncertainty on elevation and derived topographic parameters. 
