...
%DIF ADD Pulwicki_etal_2018_review.tex   Wed Mar  7 07:25:08 2018
% v4.00 3-sept-2015
...
\usepackage{color}
%DIF 22a22-23
\renewcommand{\vector}[1]{\mathbf{#1}} %DIF > 
 %DIF > 
%DIF -------
...
, }\DIFaddend Alpine3D \citep{Lehning2006} and SnowDrift3D \citep{Schneiderbauer2011} are widely used, and errors in estimating snow distribution have been examined from theoretical \citep[e.g.][]{Trujillo2015} and applied perspectives \citep[e.g.][]{Turcan1975,Woo1978, Deems2006}. 
...
so we chose a sampling design that resulted in a high ratio ($\sim$55:1) of snow depth to density measurements. In total, we collected more than 9000 snow-depth measurements and more than 100 density measurements throughout the study area (Table \ref{tab:GlacierDetails}).
}
...
S3 & $\blacksquare$ &  & \DIFdelbeginFL %DIFDELCMD < \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Regression of density on \\ elevation for a glacier ($n_T$)\end{tabular}} %%%
\DIFdelendFL \DIFaddbeginFL \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Regression of density on \\ elevation for each glacier ($n_T$)\end{tabular}} \DIFaddendFL \\
F3 &  & $\blacksquare$ &  \\ \hline
S4 & $\blacksquare$ &  & \DIFdelbeginFL %DIFDELCMD < \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Inverse distance weighted\\ mean for a glacier ($n_T$)\end{tabular}} %%%
\DIFdelendFL \DIFaddbeginFL \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Inverse distance weighted\\ mean for each glacier ($n_T$)\end{tabular}} \DIFaddendFL \\
F4 &  & $\blacksquare$ & 
\end{tabular}
\end{table}
...
}\DIFdelend \DIFaddbegin \DIFadd{, where snow depth measurements collected by one observer were, on average, greater than the snow depth measurements taken by the other two observers (p$<$0.01). Since this was the first transect and the only one to show differences by observer, this difference can be considered an anomaly. We conclude that observer bias is not an important effect in this study and therefore apply no observer corrections to the data.
}\DIFaddend 

\subsection{Distributed winter balance}

Gridcell-averaged values of \DIFdelbegin \DIFdel{WB }\DIFdelend \DIFaddbegin \DIFadd{$b_\mathrm{w}$ }\DIFaddend are interpolated and extrapolated across each glacier using linear regression (LR) and \DIFdelbegin \DIFdel{simple kriging (SK}\DIFdelend \DIFaddbegin \DIFadd{ordinary kriging (OK}\DIFaddend ). The \DIFdelbegin \DIFdel{regression }\DIFdelend \DIFaddbegin \DIFadd{LR }\DIFaddend relates gridcell-averaged \DIFdelbegin \DIFdel{WB and }\DIFdelend \DIFaddbegin \DIFadd{$b_\mathrm{w}$ to }\DIFaddend various topographic parameters\DIFdelbegin \DIFdel{, as this method }\DIFdelend \DIFaddbegin \DIFadd{. We use this method because it }\DIFaddend is simple and has precedent for success \citep[e.g.][]{McGrath2015}. Instead of a \DIFdelbegin \DIFdel{basic regression }\DIFdelend \DIFaddbegin \DIFadd{standard LR }\DIFaddend however, we use cross-validation \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{implemented in such a way as to prevent data overfitting, and employ }\DIFaddend model averaging to \DIFdelbegin \DIFdel{test }\DIFdelend \DIFaddbegin \DIFadd{allow for }\DIFaddend all combinations of the \DIFaddbegin \DIFadd{chosen }\DIFaddend topographic parameters. We compare the \DIFdelbegin \DIFdel{regression approach with simple kriging (SK), a data-driven }\DIFdelend \DIFaddbegin \DIFadd{LR approach with conventional OK, an }\DIFaddend interpolation method free of \DIFdelbegin \DIFdel{any physical interpretation \mbox{%DIFAUXCMD
\citep[e.g.][]{Hock1999}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{physical interpretation beyond the premise of spatial correlation in the data \mbox{%DIFAUXCMD
\citep[e.g.][]{Hock1999,Rasmussen2006}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

\subsubsection{Linear regression}

\DIFdelbegin \DIFdel{In the regression}\DIFdelend \DIFaddbegin \DIFadd{A multiple linear regression takes the form $\vector{y} = \vector{X} \vector{\beta} + \vector{\epsilon}$, where $\vector{y}$ is the dependent variable, the matrix $\vector{X}$ in our case contains the set of independent regressors (columns) for each spatial location (rows), $\vector{\beta}$ is the vector of regression coefficients and $\vector{\epsilon}$ is independent normal white noise with standard deviation $\sigma$ \mbox{%DIFAUXCMD
\citep[e.g.][]{Davis1986}}\hspace{0pt}%DIFAUXCMD
. 
In the LR}\DIFaddend , we use commonly \DIFdelbegin \DIFdel{applied }\DIFdelend \DIFaddbegin \DIFadd{defined }\DIFaddend topographic parameters as \DIFaddbegin \DIFadd{the regressors as }\DIFaddend in \cite{McGrath2015}, including elevation, slope, aspect, curvature, ``northness'' and a wind-redistribution parameter \DIFaddbegin \DIFadd{($Sx$ from \mbox{%DIFAUXCMD
\cite{Winstral2002}}\hspace{0pt}%DIFAUXCMD
)}\DIFaddend ; we add distance-from-centreline as an additional parameter. \DIFaddbegin \DIFadd{Topographic parameters are standardized for use in the LR. 
...
For details on data and methods used to estimate the topographic parameters see the Supplementary Material and \mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{Topographic parameters are standardized and then weighted by a set of fitted regression coefficients ($\beta_i$) calculated by minimizing the sum of squares of the vertical deviations of each datum from the regression line \mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{To }\DIFdelend %DIF > The LR implemented in this study is an extension of a multiple linear regression; we implement model averaging to incorporate every possible combination of topographic parameters and cross-validation to avoid overfitting the data. 
%DIF > 
\DIFaddbegin \DIFadd{We use a combination of cross validation and model averaging to }\DIFaddend avoid overfitting the data\DIFdelbegin \DIFdel{and to incorporate every possible combination of topographic parameters, cross-validation and model averaging are implemented. First, cross-validation is used to obtain a set of $\beta_i$ values that have the greatest predictive ability . We }\DIFdelend \DIFaddbegin \DIFadd{, to account for uncertainty in the selected predictors and to maximize the model's predictive ability \mbox{%DIFAUXCMD
\citep{Madigan1994,Kohavi1995}}\hspace{0pt}%DIFAUXCMD
.  Since there are 7 predictors, there are $2^7$ possible subsets of predictors, or equivalently, models.
%DIF > 
For a given model, we }\DIFaddend randomly select 1000 subsets of the data (\DIFaddbegin \DIFadd{where each subset includes $\sim$}\DIFaddend 2/3 of the \DIFdelbegin \DIFdel{values) to fit the LR and }\DIFdelend \DIFaddbegin \DIFadd{data) and fit a multiple linear regression using least squares (implemented in MATLAB), thus obtaining 1000 sets of $\vector{\beta}$. 
...
We }\DIFaddend use the remaining data (\DIFaddbegin \DIFadd{$\sim$}\DIFaddend 1/3 of the values) to calculate a root mean squared error (RMSE) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Kohavi1995}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{between the estimated and  observed $b_\mathrm{w}$ at the measurement locations}\DIFaddend . From the 1000 sets of \DIFdelbegin \DIFdel{$\beta_i$ }\DIFdelend \DIFaddbegin \DIFadd{$\vector{\beta}$ }\DIFaddend values, we select the set that results in the lowest RMSE. \DIFdelbegin \DIFdel{Second, we use model averaging to account for uncertainty when selecting predictors and to maximize the model's predictive ability \mbox{%DIFAUXCMD
\citep{Madigan1994}}\hspace{0pt}%DIFAUXCMD
.  Models are generated by calculating a set of $\beta_i$ as described above for all possible combinations of topographic parameters (}\DIFdelend \DIFaddbegin \DIFadd{This set of $\vector{\beta}$ has the greatest predictive ability for a particular linear combination of topographic parameters. 
The procedure above is repeated for each of the models, giving the best $\vector{\beta}$ for each of the }\DIFaddend 2$^7$ models\DIFdelbegin \DIFdel{).
Using a Bayesian framework, model averaging involves weighting all models by their posterior model probabilities \mbox{%DIFAUXCMD
\citep{Raftery1997}}\hspace{0pt}%DIFAUXCMD
.  To obtain the final regression coefficients, the $\beta_i$ values from each model are weighted according to the }\DIFdelend \DIFaddbegin \DIFadd{.
}

\DIFadd{With the $\vector{\beta}$'s in hand, we move on to prediction.  To do so, we use Bayesian model averaging. 
%DIF > Model averaging involves weighting all models by their posterior model probabilities   \citep{Raftery1997}.
We weight the models according to their }\DIFaddend relative predictive success\DIFdelbegin \DIFdel{of the model}\DIFdelend , as assessed by the value of the Bayesian Information Criterion (BIC)   \citep{Burnham2004}. 
BIC penalizes more complex models\DIFdelbegin \DIFdel{which further }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend reduces the risk of overfitting. The \DIFdelbegin \DIFdel{distributed WB is then obtained by applying the resulting regression coefficients to the topographic parameters associated with each gridcell.
}\DIFdelend \DIFaddbegin \DIFadd{final set of $\vector{\beta}$ is then the weighted sum of $\vector{\beta}$ from all models. 
...
%DIF > Distributed $b_\mathrm{w}$ is obtained by applying the final set of $\vector{\beta}$ to the topographic parameters associated with each gridcell. 
}\DIFaddend 

\subsubsection{\DIFdelbegin \DIFdel{Simple }\DIFdelend \DIFaddbegin \DIFadd{Ordinary }\DIFaddend kriging}

\DIFdelbegin \DIFdel{Simple kriging (SK) is a data-driven }\DIFdelend \DIFaddbegin \DIFadd{Kriging is a }\DIFaddend method of estimating \DIFaddbegin \DIFadd{dependent }\DIFaddend variables at unsampled locations by using the \DIFdelbegin \DIFdel{isotropic spatial correlation (covariance) }\DIFdelend \DIFaddbegin \DIFadd{spatial correlation }\DIFaddend of measured values to find a set of optimal weights \citep{Davis1986, Li2008}. 
\DIFdelbegin \DIFdel{Simple kriging }\DIFdelend \DIFaddbegin \DIFadd{Kriging }\DIFaddend assumes spatial correlation between \DIFdelbegin \DIFdel{sampling locations that are }\DIFdelend \DIFaddbegin \DIFadd{the dependent variables at the sampling locations }\DIFaddend distributed across a surface and then applies the correlation to interpolate between these locations. \DIFdelbegin \DIFdel{We used the }\texttt{\DIFdel{DiceKriging}} %DIFAUXCMD
\DIFdel{R package \mbox{%DIFAUXCMD
\citep{Roustant2012} }\hspace{0pt}%DIFAUXCMD
to calculate the maximum likelihood covariance matrix, as well as the range distance ($\theta$) and nugget for gridcell-averaged values of winter balance. The range distance is a measure of data correlation length and the nugget is the residual that encompasses sampling-error variance as well as the spatial variance at distances less than the minimum sample spacing \mbox{%DIFAUXCMD
\citep{Li2008}}\hspace{0pt}%DIFAUXCMD
.
Unlike topographic regression, simple kriging }\DIFdelend \DIFaddbegin \DIFadd{Many forms of kriging have been developed to accommodate different data types \mbox{%DIFAUXCMD
\citep[e.g.][and sources therein]{Li2008}}\hspace{0pt}%DIFAUXCMD
. Ordinary kriging (OK) }\DIFaddend is \DIFaddbegin \DIFadd{the simplest form of kriging in cases where the mean of the estimated field is unknown.
Unlike LR, OK is }\DIFaddend not useful for generating hypotheses to explain the physical controls on snow distribution, nor can it be used to estimate winter balance on unmeasured glaciers. \DIFaddbegin \DIFadd{However, we chose to use OK because it does not require external inputs and is therefore a means of obtaining $B_\mathrm{w}$ that is free of physical interpretation beyond the information contained in the covariance matrix.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdel{To quantify the uncertainty on estimates of glacier-wide WB we conduct a Monte Carlo analysis, which uses repeated random sampling of input variables to calculate a distribution of output variables \mbox{%DIFAUXCMD
\citep{Metropolis1949}}\hspace{0pt}%DIFAUXCMD
.
We repeat the random sampling process 1000 times, resulting in a distribution of values of the glacier-wide WB based on uncertainties associated with }\DIFdelend \DIFaddbegin \DIFadd{The OK model can be written $y(s) = \mu + z(s) + e$, where $\mu$ is the mean and $e$ is independent white noise with standard deviation $\sigma_e$ (also known as the nugget) that captures the sampling error as well as spatial variation at distances smaller than that observed in the sampling design \mbox{%DIFAUXCMD
\citep{Li2008}}\hspace{0pt}%DIFAUXCMD
;
...
We use the }\texttt{\DIFadd{DiceKriging}} \DIFadd{package in R \mbox{%DIFAUXCMD
\citep{Roustant2012} }\hspace{0pt}%DIFAUXCMD
to implement ordinary kriging.
For our application we employ an isotropic Mat\'ern correlation model with shape parameter $\nu=5/2$
\mbox{%DIFAUXCMD
\citep[see][]{Rasmussen2006}}\hspace{0pt}%DIFAUXCMD
. This specification implies a fairly smooth response surface (twice differentiable) and is used in many applications \mbox{%DIFAUXCMD
\citep[e.g.][]{Stein1999}}\hspace{0pt}%DIFAUXCMD
.  
The model parameters, $\mu$, $\sigma_e$, $\sigma_z$ and range parameter for the Mat\'ern correlation function are estimated using maximum likelihood.  There is no closed form solution for these parameter estimates and they are found numerically.  To ensure stability of the maximum likelihood solution, we use 500 random restarts of }\DIFaddend the \DIFdelbegin \DIFdel{four steps outlined above.  We use the standard deviation of this distribution as a useful metric of uncertainty on the glacier-wide WB.
}\DIFdelend \DIFaddbegin \texttt{\DIFadd{DiceKringing}} \DIFadd{package (each with a different initial value of the parameters).
}


%DIF > We use the \texttt{DiceKriging} package in R \citep{Roustant2012} to implement ordinary kriging with a Mat\'ern covariance function with $\nu$=5/2 
%DIF > Covariance model parameters are estimated using maximum likelihood, where multiple random restarts are required to insure stability. We specify an isotropic model [WRITE RANGE OR THETA?] and 500 restarts.  
%DIF > 
%DIF > Model parameters ($\mu$, $\sigma^2_z$, $\sigma^2_{\epsilon}$,$\theta$,nugget) are estimated with maximum likelihood using the \texttt{DiceKriging} package in R \citep{Roustant2012}. This is a challenging likelihood to maximize, so random re-starts of the package are required.
%DIF > A Mat\'ern covariance function with $\nu$=5/2 is used and we define a stationary and isotropic covariance. Covariance kernels are parameterized as in \cite{Rasmussen2006}. 

\subsection{\DIFadd{Uncertainty analysis using a Monte Carlo approach}}

\DIFaddend Three sources of uncertainty are considered separately: the uncertainty due to (1) grid-scale variability of \DIFdelbegin \DIFdel{WB }\DIFdelend \DIFaddbegin \DIFadd{$b_\mathrm{w}$ }\DIFaddend ($\sigma_{\mathrm{GS}}$), (2) the assignment of snow density ($\sigma_{\rho}$) and (3) interpolating and extrapolating gridcell-averaged values of \DIFdelbegin \DIFdel{WB }\DIFdelend \DIFaddbegin \DIFadd{$b_\mathrm{w}$ }\DIFaddend ($\sigma_{\mathrm{INT}}$). \DIFdelbegin \DIFdel{These individual }\DIFdelend \DIFaddbegin \DIFadd{To quantify the combined uncertainty due to grid-scale variability, method of density assignment and interpolation uncertainty on estimates of $B_\mathrm{w}$ we conduct a Monte Carlo analysis that uses repeated random sampling of input variables to calculate a distribution of output variables \mbox{%DIFAUXCMD
\citep{Metropolis1949}}\hspace{0pt}%DIFAUXCMD
. We repeat the random sampling process 1000 times, resulting in a distribution of values of $B_\mathrm{w}$ based on uncertainties associated with the four steps we implement to derive $B_\mathrm{w}$ from distributed snow-depth and density measurements. 
Individual }\DIFaddend sources of uncertainty are propagated through the conversion of snow depth and density measurements to \DIFdelbegin \DIFdel{glacier-wide WB}\DIFdelend \DIFaddbegin \DIFadd{$B_\mathrm{w}$}\DIFaddend . Finally, the combined effect of all three sources of uncertainty on \DIFdelbegin \DIFdel{the glacier-wide WB }\DIFdelend \DIFaddbegin \DIFadd{$B_\mathrm{w}$ }\DIFaddend is quantified. We \DIFdelbegin \DIFdel{calculate a relative uncertaintyas the normalized sum of differences between every pair of one hundred distributed WB estimates including }\DIFdelend \DIFaddbegin \DIFadd{use the standard deviation of the distribution of $B_\mathrm{w}$ as a useful metric of $B_\mathrm{w}$ uncertainty. Density assignment uncertainty is calculated as the standard deviation of the eight resulting values of $B_\mathrm{w}$. To investigate the spatial patterns in $b_\mathrm{w}$ uncertainty, we calculate a combined uncertainty, which is equal to the square root of the summed variance of distributed $b_\mathrm{w}$ that arises from }\DIFaddend $\sigma_{\mathrm{GS}}$\DIFaddbegin \DIFadd{, $\sigma_{\mathrm{\rho}}$ }\DIFaddend and $\sigma_{\mathrm{INT}}$. \DIFaddbegin \DIFadd{See Supplementary Material (Figs. S5 and S6) for plots of standard deviation of distributed $b_\mathrm{w}$ arising from individual sources of uncertainty.
}\DIFaddend 
...
\DIFdelend \DIFaddbegin \begin{figure}
	...
	\caption{\DIFaddFL{Combined uncertainty of distributed winter balance ($b_\mathrm{w}$) for density-assignment method S2 (Fig. \ref{fig:LR_OK_map}) found using linear regression (top row) and ordinary kriging (bottom row). Ice-flow directions are indicated by arrows.}}
...
%DIF > because OK is a data-fitting algorithm. However, the variance explained (Fig. \ref{fig:observedVSestimated_S2}) for Glacier 4 is comparatively low.
%DIF >  and RMSE is comparatively high (Table \ref{tab:WSMB&RMSE}).
}
...
\ref{fig:LR_OK_map}). 
}\DIFdelend \DIFaddbegin \DIFadd{. The snow on Glacier 4 also did not appear to have been affected by melt and it is hypothesized that significant wind-redistribution of snow, which was not captured by the $Sx$ parameter, covered ice-topography and produced a relatively uniform snow depth across the glacier.
}\DIFaddend 
