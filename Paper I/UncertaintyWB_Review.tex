...
Individual sources of uncertainty are propagated through the conversion of snow depth and density measurements to $B_\mathrm{w}$. Finally, the combined effect of all three sources of uncertainty on $B_\mathrm{w}$ is quantified. We use the standard deviation of the distribution of $B_\mathrm{w}$ as a useful metric of $B_\mathrm{w}$ uncertainty. Density assignment uncertainty is calculated as the standard deviation of the eight resulting values of $B_\mathrm{w}$. We calculate a relative uncertainty, equal to the normalized sum of differences between every pair of one hundred distributed $b_\mathrm{w}$ estimates including $\sigma_{\mathrm{GS}}$ and $\sigma_{\mathrm{INT}}$, to investigate the spatial patterns in $b_\mathrm{w}$ uncertainty.
