...
%The LR implemented in this study is an extension of a multiple linear regression; we implement model averaging to incorporate every possible combination of topographic parameters and cross-validation to avoid overfitting the data. 
%
...
%Model averaging involves weighting all models by their posterior model probabilities   \citep{Raftery1997}.
...
%Distributed $b_\mathrm{w}$ is obtained by applying the final set of $\vector{\beta}$ to the topographic parameters associated with each gridcell. 
...
%We use the \texttt{DiceKriging} package in R \citep{Roustant2012} to implement ordinary kriging with a Mat\'ern covariance function with $\nu$=5/2 
%Covariance model parameters are estimated using maximum likelihood, where multiple random restarts are required to insure stability. We specify an isotropic model [WRITE RANGE OR THETA?] and 500 restarts.  
%
%Model parameters ($\mu$, $\sigma^2_z$, $\sigma^2_{\epsilon}$,$\theta$,nugget) are estimated with maximum likelihood using the \texttt{DiceKriging} package in R \citep{Roustant2012}. This is a challenging likelihood to maximize, so random re-starts of the package are required.
...
%because OK is a data-fitting algorithm. However, the variance explained (Fig. \ref{fig:observedVSestimated_S2}) for Glacier 4 is comparatively low.
% and RMSE is comparatively high (Table \ref{tab:WSMB&RMSE}).
